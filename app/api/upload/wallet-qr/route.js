...
export async function POST(req) {
  ...
  try {
    ...
    const method = String(formData.get('method') || 'wallet').replace(/[^a-z0-9-_]/gi, '-')
    if (!file || typeof file === 'string') {
      return Response.json({ error: 'No file provided' }, { status: 400 })
    }

    const { name, size, type } = file
    if (size > MAX_SIZE) return Response.json({ error: 'File too large (max 2MB)' }, { status: 400 })
    ...
    const path = `${method}-${Date.now()}.${safeExt}`
    ...
    if (error) {
      if (error.message?.includes('Bucket not found') || error.message?.includes('does not exist')) {
        return Response.json({
          error: 'Storage bucket "wallets" not found. Create it in Supabase Dashboard: Storage > New bucket > name "wallets" > Public.',
        }, { status: 500 })
      }
