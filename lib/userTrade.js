...
export function isCopySubscription(row) {
  return parseTradeNotes(row?.notes).kind === 'copy_subscription'
}

export function isUserPlacedTrade(row) {
  if (isCopySubscription(row)) return false
  const notes = parseTradeNotes(row?.notes)
  return notes.kind === 'user_trade' || ['buy', 'sell'].includes(String(row?.result || '').toLowerCase())
}

export function getTradeStatus(row) {
  const notes = parseTradeNotes(row?.notes)
  if (notes.status) return String(notes.status).toLowerCase()
  if (isUserPlacedTrade(row)) return 'open'
  return 'closed'
}
