...
export default function GlobalMarketsCards() {
  return (
    <section className="py-12 lg:py-20 border-t border-dark-border bg-dark-card/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-w-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
          {items.map(({ title, points, cta }) => (
            <div key={title} className="p-5 lg:p-6 rounded-xl bg-dark-card border border-dark-border min-w-0">
              <h3 className="text-xl font-bold text-white mb-4 leading-tight">{title}</h3>
              <ul className="space-y-2 mb-6">
                {points.map((point) => (
                  <li key={point} className="flex items-start gap-2 text-slate-400 text-sm leading-relaxed">
                    <CheckIcon />
                    <span>{point}</span>
                  </li>
                ))}
              </ul>
              <Link href="/platform" className="inline-flex items-center gap-1 px-4 py-2.5 rounded-lg bg-slate-700/50 text-slate-300 hover:bg-slate-600/50 text-sm font-medium">
                {cta} →
              </Link>
            </div>
          ))}
